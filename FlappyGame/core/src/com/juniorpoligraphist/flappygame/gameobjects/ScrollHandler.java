...
public class ScrollHandler {
    ...
    public boolean collides(Gyrocopter gyrocopter) {
        ...
        if (!pipe1.isScored()
                && pipe1.getX() + (pipe1.getWidth() / 2) < gyrocopter.getX()
                + gyrocopter.getWidth()) {
            addScore(1);
            pipe1.setScored(true);
            AssetLoader.coin.play();
        } else if (!pipe2.isScored()
                && pipe2.getX() + (pipe2.getWidth() / 2) < gyrocopter.getX()
                + gyrocopter.getWidth()) {
            addScore(1);
            ...
            AssetLoader.coin.play();
                ...
                && pipe3.getX() + (pipe3.getWidth() / 2) < gyrocopter.getX()
                + gyrocopter.getWidth()) {
            addScore(1);
        ...
        return (pipe1.collides(gyrocopter) || pipe2.collides(gyrocopter) || pipe3
                .collides(gyrocopter));
    }
